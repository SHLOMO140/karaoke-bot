...
async def on_chord_key(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if vid not in context.user_data.get("chords", {}):
        # The chord keyboard now lives on a photo message, which has no
        # editable text — _respond falls back to a fresh text message there.
        await _respond(query, "האקורדים לא זמינים יותר, בחר שיר שוב.")
        return
